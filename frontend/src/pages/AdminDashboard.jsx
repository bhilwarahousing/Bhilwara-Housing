...
export default function AdminDashboard() {
  ...
  const handleEditPropSubmit = async (e) => {
    ...
    try {
      await adminAPI.updateProperty(editingProperty.id, {
      ...
      setEditPropSuccess('Property details updated successfully!');
      setTimeout(() => {
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-navy-950 text-gray-900 dark:text-gray-100 flex flex-col md:flex-row">
                    ...
                    >
                      ...
                      <div className="w-full lg:w-48 flex flex-row lg:flex-col gap-2 shrink-0 pt-2 lg:pt-0 border-t lg:border-t-0 border-gray-100">
                        <button
                          ...
                          className="flex-1 bg-green-600 hover:bg-green-700 text-white font-bold text-xs py-2.5 rounded-xl flex items-center justify-center gap-2 transition-colors shadow"
                        >
                        ...
                        <button
                          ...
                          className="flex-1 bg-indigo-50 hover:bg-indigo-600 text-indigo-700 hover:text-white font-bold text-xs py-2 rounded-xl border border-indigo-200 flex items-center justify-center gap-2 transition-colors"
                        >
                        ...
                        <button
                          ...
                          className="flex-1 bg-red-50 hover:bg-red-600 text-red-600 hover:text-white font-bold text-xs py-2 rounded-xl border border-red-200 hover:border-red-600 flex items-center justify-center gap-2 transition-colors"
                        >
      ...
      {editingProperty && (
        <div className="modal-overlay fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="modal-panel relative w-full max-w-2xl bg-white rounded-3xl p-7 shadow-2xl border border-gray-200 z-10 my-6 animate-slideUp max-h-[90vh] overflow-y-auto">
            <button
            ...
            <div className="flex items-center gap-2.5 mb-2">
              ...
              <div>
                <h3 className="font-serif text-xl font-bold text-navy-900">Edit Property Details</h3>
                <p className="text-gray-500 text-xs">Update listing #{editingProperty.id} — {editingProperty.title}</p>
              </div>
            </div>
            ...
            {editPropSuccess && (
              <div className="mt-4 p-3 bg-green-50 border border-green-200 text-green-700 rounded-xl text-xs flex items-center gap-2">
            ...
            <form onSubmit={handleEditPropSubmit} className="mt-5 space-y-4">
              ...
              <div>
                <label className="block text-xs font-bold text-gray-700 uppercase mb-1.5">Listing Status (Text & Interactive Selector)</label>
                <div className="flex flex-wrap gap-2 mb-2">
                  {[
                    { status: 'APPROVED', label: 'APPROVED (Live)', color: 'bg-green-100 text-green-800 border-green-300' },
                    { status: 'PENDING', label: 'PENDING (Review)', color: 'bg-amber-100 text-amber-800 border-amber-300' },
                    { status: 'REJECTED', label: 'REJECTED', color: 'bg-red-100 text-red-800 border-red-300' },
                    { status: 'ARCHIVED', label: 'ARCHIVED', color: 'bg-gray-100 text-gray-800 border-gray-300' },
                    { status: 'SOLD', label: 'SOLD', color: 'bg-purple-100 text-purple-800 border-purple-300' },
                    { status: 'RENTED', label: 'RENTED', color: 'bg-indigo-100 text-indigo-800 border-indigo-300' },
                  ].map((st) => (
                    <button
                      key={st.status}
                      type="button"
                      ...
                      className={`px-3 py-1.5 rounded-xl text-xs font-bold border transition-all ${
                        editPropForm.status === st.status
                          ? `${st.color} shadow-sm ring-2 ring-indigo-500 scale-105`
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div>
                  <label className="block text-xs font-bold text-gray-700 uppercase mb-1">Listing Type</label>
                  <select
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block text-xs font-bold text-gray-700 uppercase mb-1">Area (sq.ft)</label>
                  <input
                    type="number"
                    required
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div>
                  <label className="block text-xs font-bold text-gray-700 uppercase mb-1">Bedrooms (BHK)</label>
                  <input
                    type="number"
                ...
                <div>
                  <label className="block text-xs font-bold text-gray-700 uppercase mb-1">Total Floors (Floor No.)</label>
                  <input
                    type="number"
                    value={editPropForm.total_floors}
                    onChange={(e) => setEditPropForm({ ...editPropForm, total_floors: e.target.value })}
                    placeholder="e.g. 1, 2, 3"
                    className="input-field text-xs font-bold text-navy-900"
                  />
                </div>
              </div>

              <div>
                <label className="block text-xs font-bold text-gray-700 uppercase mb-1">Street Address / Locality</label>
                <input
                  type="text"
                  required
              ...
              <div className="bg-gray-50 p-4 rounded-2xl border border-gray-200 space-y-3">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold text-gray-700 uppercase flex items-center gap-1.5">
                    <MapPin size={14} className="text-indigo-600" /> Map Coordinates & Locality
                  </span>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-[11px] font-bold text-gray-500 uppercase mb-0.5">Latitude</label>
                    <input
                      type="number"
                      step="any"
                  ...
                  <div>
                    <label className="block text-[11px] font-bold text-gray-500 uppercase mb-0.5">Longitude</label>
                    <input
                      type="number"
                      step="any"
                ...
                <div className="flex items-center gap-1.5 flex-wrap pt-1">
                  <span className="text-[10px] text-gray-500 font-bold uppercase">Quick Set:</span>
                  {[
                    ...
                    <button
                      ...
                      className="text-[10px] bg-white border border-gray-300 hover:border-indigo-500 hover:text-indigo-600 px-2.5 py-1 rounded-lg font-bold transition-colors"
                    >
                      📍 {loc.name}
                    </button>
                  ))}
              ...
              <div className="bg-gray-50 p-4 rounded-2xl border border-gray-200 space-y-3">
                <label className="block text-xs font-bold text-gray-700 uppercase">Manage Property Images ({editPropForm.images.length})</label>
                
                {/* Upload File or Add URL */}
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  <div>
                    <label className="block text-[11px] font-bold text-gray-500 mb-1">Upload New Image File</label>
                    <input
                      type="file"
                      accept="image/*"
                      onChange={handleFileUpload}
                      disabled={editPropForm.uploadingImage}
                      className="text-xs text-gray-600 file:mr-2 file:py-1.5 file:px-3 file:rounded-xl file:border-0 file:text-xs file:font-bold file:bg-indigo-600 file:text-white hover:file:bg-indigo-700 cursor-pointer"
                    />
                    {editPropForm.uploadingImage && <span className="text-[11px] text-indigo-600 font-bold ml-2">Uploading image...</span>}
                  </div>

                  <div>
                    <label className="block text-[11px] font-bold text-gray-500 mb-1">Or Add Image Web URL</label>
                    <div className="flex gap-1.5">
                      ...
                      <button
                        ...
                        className="px-3 py-1.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-xs font-bold shrink-0"
                      >
                        Add URL
                      </button>
                    </div>
                ...
                {editPropForm.images.length === 0 ? (
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-2">
                    {editPropForm.images.map((imgUrl, idx) => (
                      <div key={idx} className="relative group rounded-xl overflow-hidden border border-gray-200 bg-white aspect-video shadow-sm">
